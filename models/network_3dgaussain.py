...
from models.utils import eval_sh, SH2RGB, RGB2SH
...
class GSNetwork:
    ...
    def initialize_from_mesh(self, path, R_path, radius):
        ...
        print("Number of points at loading : ", num_pts)
    ...
    def densify_and_prune_with_mask(self, max_grad, min_opacity, extent, max_screen_size, old_mask):
        ...
        if max_screen_size:
            ...
            big_points_ws = self.get_scaling.max(dim=1).values > self.percent_dense * extent
            prune_mask = torch.logical_or(torch.logical_or(prune_mask, big_points_vs), big_points_ws)
            prune_mask[new_mask==0]=False #非优化区域不会被过滤
        self.prune_points(prune_mask)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, mask=None):
        if mask != None:
            ...
            mask_clone[mask_clone>0] *= update_filter
            self.xyz_gradient_accum[mask_clone>0] += torch.norm(viewspace_point_tensor.grad[mask_clone>0][:, :2], dim=-1, keepdim=True)
            self.denom[mask_clone>0] += 1
        else:
